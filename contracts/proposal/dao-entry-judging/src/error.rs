...
pub enum ContractError {
    ...
    #[error("Proposal is ({size}) bytes, must be <= ({max}) bytes")]
    VoteTooLarge { size: u64, max: u64 },

    #[error("Proposal is ({size}) bytes, must be <= ({max}) bytes")]
    VoteTooSmall { size: u64, max: u64 },

    #[error("Already have broadcasted the maximum proposals available.")]
    TooManyProposals { size: u64, max: u64 },
